...
class FilemanagerController {
  ...
  public function execute() {
    $task = isset($_REQUEST['task']) ? WDW_S_Library::esc_sanitize_data($_REQUEST, 'task', 'sanitize_text_field') : 'display';
  ...
  public function make_dir() {
    $input_dir = (isset($_REQUEST['dir']) ? str_replace('\\', '', WDW_S_Library::esc_sanitize_data($_REQUEST, 'dir', 'sanitize_text_field')) : '');
    ...
    $args = array(
      ...
      'height' => '500',
      'task' => 'display',
      'extensions' => (isset($_REQUEST['extensions']) ? WDW_S_Library::esc_sanitize_data($_REQUEST, 'extensions', 'sanitize_text_field') : 'jpg,jpeg,png,gif'),
      'callback' => (isset($_REQUEST['callback']) ? WDW_S_Library::esc_sanitize_data($_REQUEST, 'callback', 'sanitize_text_field') : 'wds_add_image'),
      'image_for' => (isset($_REQUEST['image_for']) ? esc_html($_REQUEST['image_for']) : 'add_slides'),
      'slide_id' => esc_html($_REQUEST['slide_id']),
      'dir' => $this->esc_dir(WDW_S_Library::esc_sanitize_data($_REQUEST, 'dir', 'sanitize_text_field')),
      'TB_iframe' => '1',
    );
  ...
  public function rename_item() {
    $input_dir = (isset($_REQUEST['dir']) ? str_replace('\\', '', WDW_S_Library::esc_sanitize_data($_REQUEST, 'dir', 'sanitize_text_field')) : '');
    $input_dir = sanitize_text_field($input_dir);
    $input_dir = $this->esc_dir($input_dir);
  ...
  public function import_items() {
    $args = array(
      ...
      'import' => 'true',
      'redir' => $this->esc_dir(WDW_S_Library::esc_sanitize_data($_REQUEST, 'dir', 'sanitize_text_field')),
      'dir' => $this->get_uploads_dir() . '/' . $this->esc_dir(WDW_S_Library::esc_sanitize_data($_REQUEST, 'dir', 'sanitize_text_field')) . '/',
    );
    $query_url = wp_nonce_url(admin_url('admin-ajax.php'), 'addImage', 'nonce_wd');
    $query_url = add_query_arg($args, $query_url);
    header('Location: ' . $query_url);
